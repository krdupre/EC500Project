...
void momentum(double** eta_old, double** eta_new, double** u, double** v,  double dt, int start, int end)
{
	...
	for (j = 0; j < N; j++)
	{
	    eta_new[i][j] = eta_old[i][j] - dt/2*(eta_old[(i+1)%N][j]*u[(i+1)%N][j]-eta_old[(N+i-1)%N][j]*u[(N+i-1)%N][j])
		                          - dt/2*(eta_old[i][(j+1)%N]*v[i][(j+1)%N]-eta_old[i][(N+j-1)%N]*v[i][(N+j-1)%N]);
	}
    }
}
